use omnivers3_runtimes_stdio::{ StdinCommands, StdinEvents, StdinErrors };
use omnivers3_runtimes_stdio::console;
// use omnivers3_runtimes_stdio::linereader;
use omnivers3_runtimes_stdio::mocklinereader;
use omnivers3_systems_actor::IntoActorSystem;
use sink::{ Sink, Sink2 };
use sink::fnsink::{ FnSink };
// use omnivers3_runtimes_stdio::*;
...
pub fn main() {
    ...
    let events = FnSink::new(|e: Result<StdinEvents, StdinErrors>| {
        match e {
            Ok (event) => {
    ...
    let system1 = reader.bind(&events);

    let root = console::Config::new().bind(&system1);
    root.send(());
    ...
    // system1.send(StdinCommands::Initialize);
    // system1.send(StdinCommands::Await);
    // system1.send(StdinCommands::Await);
}
